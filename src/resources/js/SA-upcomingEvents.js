...
function makeLicenseAppReqArr(data){
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        DivCol3.innerHTML = data[i][7];
        DivCol4.innerHTML = data[i][2];
        DivCol5.innerHTML = data[i][3];
        DivCol6.innerHTML = data[i][4];
        DivCol7.innerHTML = data[i][5];
        DivCol8.innerHTML = data[i][6];

        if (data[i][4] == "Open") {
            DivCol9.innerHTML = "-"
        }
        else{
            DivCol9.appendChild(DivColTag_i1);
            DivColTag_i1.classList.add("fas");
            DivColTag_i1.classList.add("fa-info-circle");

            DivColTag_i1.onclick = function(){
                window.location.href = "SA-moreDetailsUpcomingEvents.html?"+data[i][0];
            }
        }
