window.addEventListener('DOMContentLoaded',()=>{
    ...
    if(token == undefined){
      popUpFromDown("login to continue",'red')
        ...
        fetch("http://localhost:8080/OSCA_war_exploded/SongModificationServlet", options)
        .then(res => res.json())
        .then((data) => {
  ...
  function makeSongArr(data){
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      REALtempSongID.push(data[i][0]);
        ...
        DivCol6.setAttribute("data-label", "More Details");
        DivCol7.setAttribute("data-label", "Change Ownership");
        DivCol8.setAttribute("data-label", "Remove Song");
  
        DivCol6.setAttribute("id", "moreInfo");
        DivCol7.setAttribute("id", "update");
        DivCol8.setAttribute("id", "deleteMe");
        ...
        DivColTag_i2.classList.add("fad");
        DivColTag_i2.classList.add("fa-pencil");
        DivColTag_i3.classList.add("fas");
        DivColTag_i3.classList.add("fa-trash");
  
        DivCol6.appendChild(DivColTag_i1);
        DivCol7.appendChild(DivColTag_i2);
        DivCol8.appendChild(DivColTag_i3);
        ...
        DivCol1.innerHTML = data[i][1];
        DivCol2.innerHTML = data[i][2];
        DivCol3.innerHTML = data[i][3];
        ...
        DivColTag_i1.onclick = function(){
          ...
          window.location.href='OO-moreDetailSongs.html?'+songID ;
          }
        ...
        DivColTag_i2.onclick = function(){
          ...
          loading.classList.remove("hideME");
          realpage.classList.add("hideME");
          window.location.href='OO-songModification.html?'+songID ;
          }
  
          DivColTag_i3.onclick = function(){
            popUp("Do you want to request to delete this song ?")
            songID = this.parentElement.parentElement.firstChild.innerHTML;
            tempSongID = findTempID(songID)
  ...
  function findTempID(id){
    for (let i = 0; i < realSongID.length; i++) {
      if (realSongID[i] == id){
        return REALtempSongID[i];
      }
    }
  } 
